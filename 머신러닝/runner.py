...
tf.app.flags.DEFINE_string('mode', 'eval', '''Mode {train | eval | export | single | resize | pb | map-export | map-inference} DEFAULT=eval''')
...
tf.app.flags.DEFINE_string('network', 'nn_glasses', '''File Path to network.''')
tf.app.flags.DEFINE_string('pb', None, '''pb File.''')
tf.app.flags.DEFINE_string('map', None, '''Map File Path.''')
tf.app.flags.DEFINE_string('out_node', "softmax_linear/softmax_linear:0", '''Name of out node. DEFAULT=softmax_linear/softmax''')


tf.app.flags.DEFINE_string('event_dir', './tmp/MyClassifier/Eval', '''Path to events.''')
...
tf.app.flags.DEFINE_integer('num_of_classes', 2, '''Number of classes.''')
...
tf.app.flags.DEFINE_float('learning_rate', 0.003, '''Learning rate. DEFAULT = 0.005''')
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        with tf.device('/cpu:0'):
            images, labels, filenames = nn.inputs(FLAGS.data_dir, FLAGS.batch_size, FLAGS.batch_size * 100, FLAGS.scaling, True)
...
def evaluate():
    ...
    with tf.Graph().as_default() as g:
        ...
        with tf.device("/cpu:0"):
            ...
            images, labels, filenames = nn.inputs(FLAGS.data_dir, FLAGS.batch_size, FLAGS.batch_size * 100, FLAGS.scaling, False)
